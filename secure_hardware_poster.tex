...
\definecolor{YellowHL}{HTML}{F39C12}
...
\setbeamertemplate{headline}{
  \begin{beamercolorbox}[wd=\paperwidth,ht=9cm,dp=0.5cm,
...
\setbeamertemplate{block end}{\vskip0.5ex\end{beamercolorbox}\vskip0.6cm}
...
\setbeamertemplate{block alerted end}{\vskip0.5ex\end{beamercolorbox}\vskip0.6cm}
...
\setbeamertemplate{block example end}{\vskip0.5ex\end{beamercolorbox}\vskip0.6cm}
...
\begin{column}{0.32\linewidth}
  ...
  \begin{block}{System Trust Hierarchy}
    ...
    \begin{tikzpicture}[
        ...
        box/.style={draw, rounded corners=4pt, minimum width=5.8cm,
...
\begin{column}{0.35\linewidth}
  ...
  \begin{block}{3 \; Key Secure Chip Technologies}
    ...
    \begin{tabular}{>{\bfseries\color{PrimaryBlue}}p{3.4cm}
                    p{8.1cm}}
      \toprule
  ...
  \begin{alertblock}{4 \; Threat Landscape \& Countermeasures}
    ...
    \begin{tabular}{>{\bfseries\color{AlertRed}}p{3.0cm}
                    p{4.0cm}  p{4.6cm}}
      \toprule
  ...
  \begin{block}{Attack Surface Overview}
    ...
    \begin{tikzpicture}[
      ...
      % attacks on left
      \node[atk, left=1.8cm of chip, yshift=1.2cm]  (a1) {Side-Channel};
      \node[atk, left=1.8cm of chip, yshift=0.4cm]  (a2) {Fault Injection};
      \node[atk, left=1.8cm of chip, yshift=-0.4cm] (a3) {Probing};
      \node[atk, left=1.8cm of chip, yshift=-1.2cm] (a4) {HW Trojans};
      % countermeasures on right
      \node[cnt, right=1.8cm of chip, yshift=1.2cm]  (c1) {Const-time Exec.};
      \node[cnt, right=1.8cm of chip, yshift=0.4cm]  (c2) {Redundancy};
      \node[cnt, right=1.8cm of chip, yshift=-0.4cm] (c3) {Active Shielding};
      \node[cnt, right=1.8cm of chip, yshift=-1.2cm] (c4) {Logic Testing};
      % arrows
...
\begin{column}{0.30\linewidth}
  ...
  \begin{exampleblock}{5 \; Future Frontiers}
    ...
    New hardware accelerators optimised for lattice-based and other
    quantum-resistant primitives (NIST PQC standards: CRYSTALS-Kyber,
    CRYSTALS-Dilithium) are being embedded directly in secure chips to
    ...
    profiles, bus activity) to detect subtle anomalies indicative of
    advanced side-channel or fault-injection campaigns.

    \medskip
    \textbf{\color{GreenOK}Secure Chiplets}\\
    As semiconductor manufacturing shifts toward multi-die assemblies,
    securing inter-chiplet communication fabrics (e.g., UCIe links)
    becomes critical to preserve the trust boundary.

    \medskip
    \textbf{\color{GreenOK}Blockchain-Based Device Identity}\\
    Decentralised trust models anchor device identity to PUF-derived
    hardware fingerprints registered on distributed ledgers, enabling
    scalable IoT authentication without centralised PKI.

    \medskip
    \textbf{\color{GreenOK}Self-Healing Hardware}\\
    Adaptive circuits dynamically alter clock rates, execution paths,
    or introduce deliberate jitter when an ongoing attack is detected,
    preventing data leakage without system shutdown.
  \end{exampleblock}

  % ---- Key Takeaways ----
  \begin{block}{6 \; Key Takeaways}
    \begin{itemize}[leftmargin=1.2em, itemsep=0.5ex]
      ...
      \item[\color{PrimaryBlue}\textbullet]
        ...
        of computation; countermeasures must be co-designed with the
        silicon.
      \item[\color{PrimaryBlue}\textbullet]
      ...
      \item[\color{PrimaryBlue}\textbullet]
        ...
        architectures defines the next decade of secure hardware
        research.
    \end{itemize}
  \end{block}
